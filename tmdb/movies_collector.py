    ...
    for index, movie_id in enumerate(all_ids):
        ...
        try:
            ...
            if response.status_code == 404:
                print(f"   Movie {movie_id} deleted/missing on TMDB.")
                continue
            ...
            for g in data.get('genres', []):
                if g['id'] in genre_map: new_movie.genres.append(genre_map[g['id']])
            
            for c in data.get('production_countries', []):
                if c['iso_3166_1'] in country_map: new_movie.countries.append(country_map[c['iso_3166_1']])
